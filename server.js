...
const WATERMARK_TEXT = process.env.WATERMARK_TEXT || 'BY @examschats';
const BANNER_TEXT = process.env.BANNER_TEXT || 'JOIN https://t.me/examschats For discussion';
const BANNER_URL = process.env.BANNER_URL || 'https://t.me/examschats';
...
/**
 * Watermark and add top/bottom banner with embedded clickable link to PDF.
 */
async function brandPdf(pdfBytes) {
    try {
        ...
        const bannerText = BANNER_TEXT;
        const bannerUrl = BANNER_URL;
        ...
        for (const page of pages) {
            ...
            const margin = 6;
            const bFontSize = 9;
            const textWidth = font.widthOfTextAtSize(bannerText, bFontSize);

            // Add embedded clickable URI link annotation
            const addLinkAnnotation = (rect) => {
                try {
                    const uriAction = pdfDoc.context.obj({
                        Type: PDFName.of('Action'),
                        S: PDFName.of('URI'),
                        URI: PDFString.of(bannerUrl),
                    });
                    ...
                    if (typeof page.node.addAnnot === 'function') {
                    ...
                    console.error('Failed to add link annotation:', annotErr.message);
                }
            };

            // 2. Top Banner (with margin)
            const topY = height - margin - bannerHeight;
            page.drawRectangle({
                x: margin,
                y: topY,
            ...
            const topTextX = Math.max(margin + 5, (width - textWidth) / 2);
            ...
            addLinkAnnotation([margin, topY, width - margin, topY + bannerHeight]);

            // 3. Bottom Banner (with margin)
            ...
            const bottomTextX = Math.max(margin + 5, (width - textWidth) / 2);
            page.drawText(bannerText, {
                x: bottomTextX,
                y: bottomY + 4.5,
                size: bFontSize,
            ...
            addLinkAnnotation([margin, bottomY, width - margin, bottomY + bannerHeight]);
        }
...
/**
 * Download PDF, brand with watermark & banners, rename, and send to Telegram.
...
async function sendPdfToChat(chatId, pdfUrl, customCaption = null) {
    try {
        ...
        addLog(`[PDF] Downloaded ${Math.round(rawBytes.length / 1024)}KB, applying watermark & banners...`, 'info');

        // Extract epoch ID from URL and construct target filename
        const match = pdfUrl.match(/(\d+)\.pdf$/i);
        const epoch = match ? match[1] : (pdfUrl.split('/').pop().replace(/\.pdf$/i, '') || String(nowEpoch()));
        ...
        // getBuffer() is synchronous — works reliably when all fields are Buffers/strings.
